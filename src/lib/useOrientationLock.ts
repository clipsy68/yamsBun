...
const LANDSCAPE_SCREENS = new Set(['general', 'end', 'selectFirst', 'historyDetail'])

/** Locks the device orientation to match the current screen — landscape for the
 * wide multi-table views (general table, game end, first-player pick, history
 * detail), portrait for everything else. No-ops silently outside the native app. */
